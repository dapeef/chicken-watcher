...
def get_dashboard_context():
    ...
    if db.connection.vendor == "postgresql":
        ...
        latest_presence = []
        box_ids = NestingBoxPresence.objects.filter(present_at__gte=today_start).values_list('nesting_box_id', flat=True).distinct()
        for b_id in box_ids:
            p = NestingBoxPresence.objects.filter(nesting_box_id=b_id, present_at__gte=today_start).order_by('-present_at').first()
            if p:
                latest_presence.append(p)

    return {
        "eggs_today": eggs_today_qs.count(),
        "laid_chickens": Chicken.objects.filter(egg__in=eggs_today_qs).distinct().order_by("name"),
...
def partial_laid_chickens(request):
    return render(request, "web_app/partials/_laid_chickens.html", get_dashboard_context())
...
def partial_latest_image(request):
    return render(request, "web_app/partials/_latest_image.html", get_dashboard_context())


def partial_latest_presence(request):
    return render(request, "web_app/partials/_latest_presence.html", get_dashboard_context())


def partial_latest_events(request):
    return render(request, "web_app/partials/_latest_events.html", get_dashboard_context())
